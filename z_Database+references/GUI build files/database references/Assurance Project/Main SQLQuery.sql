...
select 'hi'
ALTER AUTHORIZATION ON DATABASE::[Project_Prototype] TO [sa];
------------- Creating UDT ------------- 
...
as 
	...
	select @edate=[End Date],@agid=AGID,@cpid=[CPID] from Contrats  where CLID=@clid order by [End Date] -- give biggest end date
	...
	if(@edate is not null)
		begin
			set @dif = DATEDIFF(day,getdate(),@edate) -- return diff days @sdate - getdate() // system database time
			if(@dif>0)
				begin
					if(@agid != @insagid)
						begin print 'error changing ag id is not allowed' set @valid =0 end

					if(@cpid!=@inscpid)
						begin print 'error chaging company for the same type is not allowed' set @valid =0 end
...
select 23%2
insert into Contrats values (23,2,1,9,'2015/9/21','2020/9/21',2342,'nothing') 
exec ClientReport  2;
select * from Client
...
as	
	...
	-- now let's calculate if he paid all payment till now
	set @dif = DATEDIFF(MONTH,@StartDate,@insDate)
	set @difyear= CEILING(@dif/12.0)  -- take the closer larger int
...
create function paid (@CTID int)  -- return how many times i have paid for that contrats
returns int 
begin
	return (select count(*) from PayCollection where CTID=@ctid)
end

create procedure ClientDebt  -- to print all needed payment for all users
--returns table
as
	declare @clid int
